...
const numbers = [
  ...
  { value: "60fps", label: "Locked Frame Rate" },
  ...
  return (
    ...
    >
      <div className="px-6 md:px-10">
          ...
          {capabilities.map((c, i) => {
            ...
            return (
              <div
                key={c.n}
                ref={ref}
                className={`bg-black p-8 md:p-10 reveal reveal-delay-${i + 1} ${revealed ? "is-revealed" : ""}`}
              >
                ...
                <h3 className="font-display text-[28px] md:text-[36px] font-black tracking-[-0.04em] text-white leading-none mb-6">
                  {c.title}
                </h3>
                <div className="flex flex-wrap gap-2">
                  {c.tags.map((tag) => (
                    <span
                      key={tag}
                      className="font-mono text-[10px] uppercase tracking-[0.22em] text-white/55 border border-white/15 px-2.5 py-1"
                    >
                      {tag}
                    </span>
                  ))}
        ...
        <div
          ref={numbersRef}
          className={`mt-20 reveal ${numbersRevealed ? "is-revealed" : ""}`}
        >
          <div className="font-mono text-[10px] uppercase tracking-[0.32em] text-white/35 mb-10">
            By the numbers
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-px border border-white/10 bg-white/10">
            {numbers.map((n, i) => (
              <div key={n.label} className={`bg-black px-6 py-8 reveal reveal-delay-${i + 1} ${numbersRevealed ? "is-revealed" : ""}`}>
                <div className="font-display text-[36px] md:text-[48px] font-black leading-none tracking-[-0.04em] text-white">
                  {n.value}
                </div>
                <div className="mt-3 font-mono text-[9px] uppercase tracking-[0.24em] text-white/40 leading-[1.5]">
                  {n.label}
                </div>
              </div>
            ))}
