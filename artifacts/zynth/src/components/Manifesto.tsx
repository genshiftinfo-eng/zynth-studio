...
function PrincipleCard({ p, delay }: { p: typeof principles[0]; delay: string }) {
  const [ref, revealed] = useReveal<HTMLElement>();
  return (
    <article
      ref={ref}
      className={`border-t border-white/15 pt-6 reveal ${delay} ${revealed ? "is-revealed" : ""}`}
    >
      <div className="flex items-baseline justify-between">
        <span className="font-mono text-[10px] tracking-[0.32em] uppercase text-white/45">Article {p.n}</span>
        <span className="inline-block h-2 w-2 bg-white/60" />
      </div>
      <h3 className="mt-4 font-display text-[28px] md:text-[40px] leading-[1.02] font-black tracking-[-0.04em] text-white">{p.t}</h3>
  ...
  return (
    ...
    >
      ...
      <div className="relative px-6 md:px-10">
        <div ref={headRef} className={`grid grid-cols-1 md:grid-cols-12 gap-8 reveal ${headRevealed ? "is-revealed" : ""}`}>
        ...
        <div className="mt-20 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-14 md:gap-y-20">
          {principles.map((p, i) => (
            <PrincipleCard key={p.n} p={p} delay={`reveal-delay-${i + 1}`} />
          ))}
        </div>

        <div ref={statsRef} className={`mt-24 grid grid-cols-2 md:grid-cols-4 gap-6 border-t border-white/10 pt-10 reveal ${statsRevealed ? "is-revealed" : ""}`}>
          {stats.map((s, i) => (
            <div key={s.l} className={`reveal-delay-${i + 1}`}>
              <div className="font-display text-[48px] md:text-[88px] font-black leading-none tracking-[-0.045em] text-white">{s.v}</div>
              <div className="mt-3 font-mono text-[10px] uppercase tracking-[0.28em] text-white/45 max-w-[18ch]">{s.l}</div>
            </div>
          ))}
        </div>
      </div>
